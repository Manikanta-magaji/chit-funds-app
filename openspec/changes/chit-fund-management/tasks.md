...
- [ ] 18.1 Write backend unit tests for JWT utilities, UPI ID inference, and prize draw eligibility logic
- [ ] 18.2 Write API integration tests for auth endpoints (register, login, Google callback mock)
- [ ] 18.3 Write API integration tests for group creation, admin management, and contributor slot endpoints
- [ ] 18.4 Write API integration tests for installment tracking and advance-cycle enforcement rules
- [ ] 18.5 Write API integration tests for prize draw (random and manual) and payout confirmation endpoints
- [ ] 18.6 Manually test the end-to-end flow: create group → add contributors → run cycle → draw → pay → advance
...
- [ ] 19.7 Add integration tests covering partial-match suggestion behavior, duplicate display-name rejection, mapping confirmation flow, and immediate historical visibility after linking
